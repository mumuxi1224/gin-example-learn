...
func GetArticles(c *gin.Context){
	...
	if arg:=c.Query("state");arg!="" {
		...
		valid.Range(state,0,1,"state").Message("state只允许1或0")
		maps["state"]=state
	}
	...
	if arg:=c.Query("tag_id");arg!="" {
		...
		valid.Min(tagId,1,"tag_id").Message("标签ID要大于0")
		maps["tag_id"]=tagId
	}
	...
	if	!valid.HasErrors(){
		...
		data["list"]=models.GetArticleTotal(maps)
		data["total"]=models.GetArticles( util.GetPage(c) , setting.PageSize,maps )
	}else{
		for _,err:=range valid.Errors{
			logging.Info(err.Key, err.Message)
			//log.Printf("err.key：%s,err.msg:%s",err.Key,err.Message)
